...
function rowTool(row: any) {
  return row.tool_name || row.tool || "-";
}
...
export default function ReportsPage() {
  ...
  const totalBusiness = filteredRentals.reduce((sum, r) => sum + money(r), 0);
  const collections = filteredPayments.reduce(
    (sum, p) => sum + Number(p.amount || 0),
    0
  );
  const balance = totalBusiness - collections;
  ...
  const shopWiseSummary = useMemo(() => {
    ...
    filteredPayments.forEach((p) => {
      ...
      if (!map[shop]) return;
      map[shop].collections += Number(p.amount || 0);
    });

    Object.values(map).forEach((row: any) => {
      row.balance = row.business - row.collections;
    });
  ...
  const dayWiseBusiness = useMemo(() => {
    ...
    filteredRentals.forEach((r) => {
      const date = String(r.date || r.start_date || r.rental_date || "").slice(
        0,
        10
      );
      if (!date) return;
      if (!map[date]) map[date] = { date, business: 0, collections: 0 };
      map[date].business += money(r);
    });
    ...
    filteredPayments.forEach((p) => {
      ...
      if (!date) return;
      if (!map[date]) map[date] = { date, business: 0, collections: 0 };
      map[date].collections += Number(p.amount || 0);
    });

    return Object.values(map)
      .map((r: any) => ({
        ...r,
        balance: r.business - r.collections,
      }))
  ...
  const dayWiseShopBusiness = useMemo(() => {
    ...
    filteredRentals.forEach((r) => {
      ...
      if (!map[key])
        map[key] = { date, shop, business: 0, collections: 0, balance: 0 };

      map[key].business += money(r);
    });
    ...
    filteredPayments.forEach((p) => {
      ...
      map[key].collections += Number(p.amount || 0);
    });
  ...
  const toolSummary = useMemo(() => {
    ...
    filteredRentals.forEach((r) => {
      ...
      map[tool].times += 1;
      map[tool].days += Number(r.days || 0);
      map[tool].revenue += money(r);
    });

    filteredServices.forEach((s) => {
      const tool = rowTool(s);
      if (!map[tool]) {
        map[tool] = {
          tool,
          times: 0,
    ...
    return Object.values(map).map((r: any) => ({
      ...
      roi: r.serviceCost > 0 ? ((r.revenue - r.serviceCost) / r.serviceCost) * 100 : 100,
    }));
  }, [filteredRentals, filteredServices]);
  ...
  const shopPerformance = shopWiseSummary.map((s: any) => ({
    ...
    profit: s.business,
    utilization:
      tools.length > 0 ? Math.round((s.active / Math.max(tools.length, 1)) * 100) : 0,
  }));
  ...
  return (
    <main>
      ...
      <ReportTable
        ...
        headers={["Date", "Business", "Collections", "Balance"]}
      ...
      <ReportTable
        ...
        headers={["Date", "Shop", "Business", "Collections", "Balance"]}
      ...
      <ReportTable
        ...
        headers={["Customer", "Mobile", "Shop", "Business", "Collections", "Balance"]}
      ...
      <ReportTable
        ...
        headers={["Month", "Service Cost", "Maintenance", "Other Expenses", "Total Expense"]}
      ...
      <ReportTable
        ...
        headers={["Shop", "Business", "Profit", "Collections", "Active Rentals", "Utilization %"]}
            ...
            {rows.map((row: any[], index: number) => (
              <tr key={index}>
                {row.map((cell: any, i: number) => (
                  <td key={i} className={String(cell).includes("₹") ? "strong" : ""}>
                    {cell || "-"}
                  </td>
                ))}
              </tr>
